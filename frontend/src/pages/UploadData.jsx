...
import { uploadCSVFile, loadDemoData, startNewDay } from '../api';
...
export default function UploadData({ centres = [], onDataReload, setActiveTab }) {
  ...
  const handleStartNewDay = async () => {
    if (!window.confirm('Start a new collection day? Existing centre data and history will be preserved.')) return;
    ...
    try {
      await startNewDay();
      setUploadSuccess('New collection day started. Drivers can now record fresh stop collections.');
      if (onDataReload) await onDataReload();
    } catch (err) {
      setUploadError(err.message || 'Failed to start a new collection day');
    } finally {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={isUploading || isStartingDay}
            className="px-3.5 py-2 bg-sky-100 hover:bg-sky-200 text-sky-800 font-semibold text-xs rounded-xl border border-sky-200 transition-all cursor-pointer disabled:opacity-50"
          >
            {isStartingDay ? 'Starting...' : 'Start New Day'}
          </button>
          <button
      ...
      <div>
        {/* Upload Dropzone */}
        <div className="p-6 rounded-2xl bg-slate-900/80 border-2 border-dashed border-slate-700 hover:border-sky-500/50 transition-all flex flex-col items-center justify-center text-center backdrop-blur-sm">
          <input
            type="file"
            ref={fileInputRef}
