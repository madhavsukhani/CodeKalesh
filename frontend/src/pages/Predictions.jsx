...
export default function Predictions({ 
  ...
  const [modelType, setModelType] = useState('weighted');
  const [isLoading, setIsLoading] = useState(false);

  const handleModelChange = async (newType) => {
    setModelType(newType);
    setIsLoading(true);
    try {
      const data = await fetchPredictions(newType);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Algorithmic yield forecast based on 7-day baseline, yesterday intake, and 30-day seasonality.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 bg-slate-900 p-1.5 rounded-2xl border border-slate-800">
          <button
            onClick={() => handleModelChange('weighted')}
        ...
        <div className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800 backdrop-blur-sm">
          ...
          <div className="mt-2 flex items-center gap-2">
            ...
            <span className="text-lg font-bold text-white">
              {modelType === 'weighted' ? 'Weighted Moving Avg' : 'Random Forest Regressor'}
            </span>
          </div>
          <p className="text-xs text-slate-400 mt-1">
            {modelType === 'weighted' ? '0.50×7d + 0.30×yesterday + 0.20×30d' : 'Non-linear tree ensemble on lag features'}
          </p>
        </div>
