...
def init_db():
    ...
    cursor.execute("""
    CREATE TABLE IF NOT EXISTS stop_reassignments (
        centre_id TEXT PRIMARY KEY,
        original_van_id TEXT NOT NULL,
        assigned_van_id TEXT NOT NULL,
        assigned_van_name TEXT NOT NULL,
        estimated_arrival TEXT NOT NULL,
        is_second_run INTEGER NOT NULL,
...
def save_reassignments(plan: List[Dict[str, Any]], original_van_id: str):
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    now_iso = datetime.now().isoformat()
    for item in plan:
        cursor.execute("""
            INSERT OR REPLACE INTO stop_reassignments 
            (centre_id, original_van_id, assigned_van_id, assigned_van_name, estimated_arrival, is_second_run, reason, reassigned_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            item["centre_id"],
            original_van_id,
            item["assigned_van_id"],
            item["assigned_van_name"],
            item["estimated_arrival"],
            1 if item.get("is_second_run") else 0,
            item.get("reason", ""),
            now_iso
        ))
    conn.commit()
...
def get_reassigned_stops() -> Dict[str, Dict[str, Any]]:
    conn = sqlite3.connect(DB_FILE)
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM stop_reassignments")
    rows = cursor.fetchall()
    conn.close()
    return {r["centre_id"]: dict(r) for r in rows}
...
def reset_completions():
    ...
    cursor.execute("DELETE FROM stop_reassignments")
    conn.commit()
    conn.close()
